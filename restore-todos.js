...
// Script to restore weekly todos from last week to current week
...
async function restoreTodos() {
  console.log('🔄 Restoring weekly todos...\n')
  ...
  try {
    // Get todos from last week
    console.log(`📝 Fetching todos from last week (${lastWeekStart})...`)
    const { data: lastWeekTodos, error: fetchError } = await supabase
      .from('todos')
      .select('*')
      .eq('week_start', lastWeekStart)
    
    if (fetchError) {
      console.error('❌ Error fetching last week todos:', fetchError.message)
      process.exit(1)
    }
    
    if (!lastWeekTodos || lastWeekTodos.length === 0) {
      console.log('⚠️  No todos found for last week. Nothing to restore.')
      process.exit(0)
    }
    
    console.log(`✅ Found ${lastWeekTodos.length} todos from last week\n`)
    ...
    if (currentWeekTodos && currentWeekTodos.length > 0) {
      ...
      console.log('   The script will add the todos from last week as well.\n')
    }
    
    // Copy todos to current week
    console.log(`📋 Copying todos to current week (${currentWeekStart})...`)
    const todosToInsert = lastWeekTodos.map(todo => ({
      title: todo.title,
